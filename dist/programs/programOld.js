...
export class ProgramOld {
    ...
    static createFramebuffer(glCtx, width, height, textureCount, hasRenderbuffer) {
        ...
        for (let i = 0; i < textureCount; i++) {
            const texture = ProgramOld.createTexture(glCtx, width, height, null);
            glCtx.framebufferTexture2D(glCtx.FRAMEBUFFER, glCtx.COLOR_ATTACHMENT0 + i, glCtx.TEXTURE_2D, texture, 0);
            textures.push(texture);
        }
        let renderbuffer;
        if (hasRenderbuffer) {
            renderbuffer = ProgramOld.createRenderbuffer(glCtx, width, height);
...
//# sourceMappingURL=programOld.js.map