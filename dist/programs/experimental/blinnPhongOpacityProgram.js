...
export class BlinnPhongOpacityProgram extends Program {
    constructor(glCtx, geometry, material) {
        super(glCtx, new VertexShader(glCtx, BlinnPhongOpacityProgram.vertexShaderSource), new FragmentShader(glCtx, BlinnPhongOpacityProgram.fragmentShaderSource));
...
BlinnPhongOpacityProgram.vertexShaderSource = `#version 300 es
...
BlinnPhongOpacityProgram.fragmentShaderSource = `#version 300 es
    precision mediump float;
    ...
    void main(void) {
      ...
      vec4 ia = uLightAmbient * uMaterialAmbient * uMaterialDiffuse;
      vec4 id = vec4(0.0, 0.0, 0.0, 1.0);
      vec4 is = vec4(0.0, 0.0, 0.0, 1.0);

      if (lambertTerm > 0.0) {
        id = uLightDiffuse * lambertTerm * uMaterialDiffuse;
      ...
      float opacity = max(max(is.r, is.g), is.b) * (1.0 - uMaterialDiffuse.a) + uMaterialDiffuse.a;

      // Final fragment color takes into account ambient, diffuse, and specular
      fragColor = vec4(vec3(ia + id + is), opacity);
    }`;
//# sourceMappingURL=blinnPhongOpacityProgram.js.map