...
//
export class SpecularMapProgram extends Program {
    constructor(glCtx, geometry, material, lightCount) {
        super(glCtx, new VertexShader(glCtx, SpecularMapProgram.getVertexShaderSource(lightCount)), new FragmentShader(glCtx, SpecularMapProgram.getFragmentShaderSource(lightCount)));
...
//      vec3 B = normalize(vec3(model * vec4(aVertexBitangent, 0.0)));
    ...
    vec3 phongShading(int index) {
      ...
      if (lambertTerm > 0.0) {
        ...
        is = uLightInfo[index].Is * uMaterialSpecular * specular * texture(uTexSpecular, vTextureCoords);
      }
...
//# sourceMappingURL=SpecularMapProgram.js.map